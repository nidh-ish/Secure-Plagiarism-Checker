...
class GenAST:
    ...
    def generate_ast(self, filename, program_number1):
        input_tree = self.mutate(filename)
        count_l = self.count_loops(input_tree)
        count_if = self.count_if(input_tree) # type: ignore
        count_f = self.count_functions(input_tree)
        self.find_levels(input_tree)

        # filename_prognum = "Program"+program_number1+"/program"+program_number1
        filename_prognum = os.path.join("Program"+program_number1, "program"+program_number1)
        ...
        pc_1.sort
        pc_2.sort
        programpath = os.path.join("Program" + program_number1, "program" + program_number1 + ".txt")

        program_name = open(programpath, "w")
        ...
        output_file_lev1.close()
        output_file_lev2.close()
        # print("-----------------------------LEVEL1 -> LEVEL2-----------------------------------------------------")
...
if __name__ == "__main__":
    ...
    gast = GenAST()
    gast.generate_ast(filename, program_number1)